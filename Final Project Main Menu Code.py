...
def menu_loop(TITLETEXTSURF):
    sel_pos = 1
    DISPLAYSURF.fill(BLACK)
    menufont3 = pygame.font.Font("C:\Python34\EAE 1410\OdalisqueNF.ttf", 30)
    ...
    CURSORSURF = pygame.Surface((100, 100), flags=SRCALPHA, depth=32)
    ...
    while True:
        ...
        if (keys_pressed[K_DOWN] + keys_pressed[K_s] + keys_pressed[K_UP] + keys_pressed[K_w]) > 1:
            ...
            if sel_pos > 1 and (keys_pressed[K_DOWN] == 1 or keys_pressed[K_s]) == 1:
                sel_pos -= 1
            elif sel_pos == 1:
                sel_pos = 4
...
class clickAnywhereG(object):
    ...
    def fade(self, TITLETEXTSURF, menufont1, TX1):
        menufont2 = pygame.font.Font("C:\Python34\EAE 1410\OdalisqueNF.ttf", 30)
        reappear_delay = 0
        fade = 1
        while True:
            for event in pygame.event.get():
...
def main():
    menufont1 = pygame.font.Font("C:\Python34\EAE 1410\OdalisqueNF.ttf", 50)
    while True:
        for event in pygame.event.get():
            if event.type == QUIT:
                pygame.quit()
